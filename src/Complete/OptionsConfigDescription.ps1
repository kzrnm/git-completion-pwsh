function Get-GitConfigDescription {
    ...
    switch -Wildcard ($Current) {
        ...
        "receive.procReceiveRefs" { 'This is a multi-valued variable that defines reference prefixes to match the commands in receive-pack.' }"receive.updateServerInfo" { "If set to true, git-receive-pack will run git-update-server-info after receiving data from git-push and updating refs." }
        "receive.shallowUpdate" { 'If set to true, .git/shallow can be updated when new refs require new shallow roots.' }
        "remote.pushDefault" { 'The remote to push to by default.' }"remote.<name>.url" { 'The URL of a remote repository.' }
        ...
        "repack.packKeptObjects" { 'If set to true, makes git repack act as if --pack-kept-objects was passed.' }"repack.useDeltaIslands" { 'If set to true, makes git repack act as if --delta-islands was passed.' }
