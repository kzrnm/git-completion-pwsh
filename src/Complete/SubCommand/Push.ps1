...
function Complete-GitSubCommand-push {
    ...
    if (!$Context.HasDoubledash()) {
        ...
        switch ($Prev) {
            ...
            '--recurse-submodules' {
                $script:gitPushRecurseSubmodules.GetEnumerator() | completeTipTable -Current $Current -ResultType ParameterValue 
                return
            }
        }
        ...
        switch -Wildcard ($Current) {
            ...
            '--recurse-submodules=*' { 
                $script:gitPushRecurseSubmodules.GetEnumerator() | completeTipTable -Current $Current -ResultType ParameterValue -Prefix '--recurse-submodules=' -RemovePrefix
                return
            }
            '--force-with-lease=*' {
